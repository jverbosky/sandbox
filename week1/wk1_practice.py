...
def circlesIntersect(x1, y1, r1, x2, y2, r2):
    # if the distance is < the sum of the two radii - intersection at 2 points
    # if the distance is = the sum of the two radii - intersection at 1 point
...
def pascalsTriangleValue(row, col):
    if (row < 0) or (col < 0) or ((row - col) < 0):
        return None
    else:
        # use backslash to continue statement on next line (80 char limit)
...
def isFactor(f, n):
    if (f == 0) and (n != 0): return False
    if (n == 0) or (n % f == 0): return True
    else: return False

def isMultiple(m, n):
    return isFactor(n, m)  # opposite of isFactor

def isLegalTriangle(s1,s2,s3):
    longest = max(s1, s2, s3)
    if (s1 + s2) > longest \
    and (s2 + s3) > longest \
    and (s3 + s1) > longest: return True
    else: return False

def triangleArea(s1, s2, s3):
    if isLegalTriangle(s1, s2, s3):
        # Heron's formula
        sp = (s1 + s2 + s3) / 2  # sp = semiperimeter
...
def nthFibonacciNumber(n):
    if n == 0: n = 1  # treat 0 as 1
    if n > 1: n += 1  # increment n so series flows correctly
    # Binet's Fibonacci number formula
    dividend_1 = (1 + math.sqrt(5))**n
    dividend_2 = (1 - math.sqrt(5))**n
    divisor = 2**n * math.sqrt(5)
    return int((dividend_1 - dividend_2) // divisor)

def isEvenPositiveInt(x):
    # tried evaluating type but linter complained:  type(x) == int
    if isinstance(x, int) \
    and x > 0 \
    and x % 2 == 0: return True
    else: return False
